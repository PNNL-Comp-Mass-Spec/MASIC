...
#Region "Classwide Variables"
	...
	Private mSICPeakFinderOptions As MASICPeakFinder.clsMASICPeakFinder.udtSICPeakFinderOptionsType
...
#End Region
	...
	Private Sub AutoOpenMsMsResults(ByVal strMASICFilePath As String, ByRef objProgress As ProgressFormNET.frmProgress)
	...
	Private Sub DisplaySICStats(ByVal intParentIonIndex As Integer, ByRef udtSICStats As udtSICStatsType)
		...
		If intParentIonIndex >= 0 And intParentIonIndex < mParentIonCount Then
			...
			With udtSICStats
				...
				If blnValidPeakFound Then
					...
					End If
					strStats &= ControlChars.NewLine & "Intensity: " & MathUtilities.ValueToString(sngIntensityToDisplay, 4)
					strStats &= ControlChars.NewLine & "Area: " & MathUtilities.ValueToString(sngAreaToDisplay, 4)
				...
				If blnValidPeakFound Then
					With .Peak.StatisticalMoments
						strStats &= ControlChars.NewLine & "Peak StDev: " & MathUtilities.ValueToString(.StDev, 3)
						strStats &= ControlChars.NewLine & "Peak Skew: " & MathUtilities.ValueToString(.Skew, 4)
						strStats &= ControlChars.NewLine & "Peak KSStat: " & MathUtilities.ValueToString(.KSStat, 4)
					...
					Else
						strStats &= ControlChars.NewLine & "S/N: " & MathUtilities.ValueToString(.Peak.SignalToNoiseRatio, 4)
					End If

					With .Peak.BaselineNoiseStats
						strStats &= ControlChars.NewLine & "Noise level: " & MathUtilities.ValueToString(.NoiseLevel, 4)
						strStats &= ControlChars.NewLine & "Noise StDev: " & MathUtilities.ValueToString(.NoiseStDev, 3)
	...
	Private Sub FindMinimumPotentialPeakAreaInRegion(ByVal intParentIonIndexStart As Integer, ByVal intParentIonIndexEnd As Integer, ByRef udtSICPotentialAreaStatsForRegion As MASICPeakFinder.clsMASICPeakFinder.udtSICPotentialAreaStatsType)
		...
		For intParentIonIndex = intParentIonIndexStart To intParentIonIndexEnd
			With mParentIonStats(intParentIonIndex)
				...
				If .SICStats.SICPotentialAreaStatsForPeak.MinimumPotentialPeakArea = 0 And _
				...
				With .SICStats.SICPotentialAreaStatsForPeak
					...
					If .MinimumPotentialPeakArea > 0 And .PeakCountBasisForMinimumPotentialArea >= MASICPeakFinder.clsMASICPeakFinder.MINIMUM_PEAK_WIDTH Then
						...
						Else
							If .MinimumPotentialPeakArea < udtSICPotentialAreaStatsForRegion.MinimumPotentialPeakArea And _
	...
	Private Function FindSICPeakAndAreaForParentIon(ByVal intParentIonIndex As Integer, ByRef udtSICStats As udtSICStatsType) As Boolean
		...
		Try
			...
			With mParentIonStats(intParentIonIndex)
				...
				Else
					...
					End If
				End If
				blnSuccess = mMASICPeakFinder.FindSICPeakAndArea(.DataCount, .SICScans, .SICData, .SICStats.SICPotentialAreaStatsForPeak, udtSICStats.Peak, _
																 udtSmoothedYDataSubset, mSICPeakFinderOptions, _
	...
	Private Sub FindSimilarParentIon(ByVal dblSimilarMZTolerance As Double)
				...
				Else
					...
					For intIndexCompare = 0 To mParentIonCount - 1
						With mParentIonStats(intIndexCompare)
							If intIndexCompare <> intParentIonIndex AndAlso _
								...
								If blnAddScan Then
									...
									If intMatchIndex < 0 Then
										...
										For intScanIndex = 0 To .DataCount - 2
											If .SICScans(intScanIndex) <= intSimilarFragScans(intFragIndex) AndAlso _
	...
	Private Function GetSettingVal(ByVal strAppName As String, ByVal strSectionName As String, ByVal strKey As String, ByVal DefaultValue As Boolean) As Boolean
	...
	Private Function GetSettingVal(ByVal strAppName As String, ByVal strSectionName As String, ByVal strKey As String, ByVal DefaultValue As Integer) As Integer
		Dim strValue As String
	...
	Private Function GetSettingVal(ByVal strAppName As String, ByVal strSectionName As String, ByVal strKey As String, ByVal DefaultValue As Single) As Single
		Dim strValue As String
	...
	Private Function InterpolateY(ByRef sngInterpolatedYValue As Single, ByVal X1 As Integer, ByVal X2 As Integer, ByVal Y1 As Single, ByVal Y2 As Single, ByVal intTargetX As Integer) As Boolean
	...
	Private Sub JumpToScan(Optional ByVal intScanNumberToFind As Integer = -1)
	...
	Private Function LookupSequenceForParentIonIndex(ByVal intParentIonIndex As Integer) As String
	...
	Private Function LookupSequenceID(ByVal strSequence As String, ByVal strProtein As String) As Integer
	...
	Private Sub NavigateScanList(ByVal blnMoveForward As Boolean)
	...
	Private Sub PlotData(ByVal intIndexToPlot As Integer, ByRef udtSICStats As udtSICStatsType)
	...
	Private Sub PopulateSpectrumList(ByVal intScanNumberToHighlight As Integer)
	...
	Private Sub ReadDataFileXMLTextReader(ByVal strFilePath As String)
	...
	Private Sub ReadMsMsSearchEngineResults(ByVal strFilePath As String, ByRef objProgress As ProgressFormNET.frmProgress)
	...
	Private Sub SortData()
		...
		If chkFilterByMZ.Checked AndAlso SharedVBNetRoutines.VBNetRoutines.IsNumber(txtFilterByMZ.Text) AndAlso _
	...
	Private Function SortDataFilterCheck(ByVal sngPeakMaxIntensityValue As Single, ByVal sngPeakSN As Single, ByVal dblPeakMZ As Double, ByVal sngMinimumIntensity As Single, ByVal sngMinimumSN As Single, ByVal sngMZFilter As Single, ByVal sngMZFilterTol As Single, ByVal blnIsCustomSIC As Boolean) As Boolean
	...
	Private Function TestValueToStringWork(ByVal sngValue As Single, ByVal intDigitsOfPrecision As Integer) As String
		Return sngValue.ToString & ": " & MathUtilities.ValueToString(sngValue, intDigitsOfPrecision)
	End Function

	Private Sub ToggleAutoStep(Optional ByVal blnForceDisabled As Boolean = False)
	...
	Private Function UpdateSICStats(ByVal intParentIonIndex As Integer, ByVal blnRepeatPeakFinding As Boolean, ByVal eSmoothMode As eSmoothModeConstants, ByRef udtSICStats As udtSICStatsType) As Boolean
...
#Region "Checkboxes"
	Private Sub chkFilterByIntensity_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkFilterByIntensity.CheckedChanged
		SortData()
	End Sub

	Private Sub chkFilterByMZ_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkFilterByMZ.CheckedChanged
		SortData()
	End Sub

	Private Sub chkFilterBySignalToNoise_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkFilterBySignalToNoise.CheckedChanged
		SortData()
	End Sub

	Private Sub chkFixXRange_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkFixXRange.CheckedChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub chkFixYRange_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkFixYRange.CheckedChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub cmdRedoSICPeakFindingAllData_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdRedoSICPeakFindingAllData.Click
		RedoSICPeakFindingAllData()
	End Sub

	Private Sub chkShowBaselineCorrectedStats_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkShowBaselineCorrectedStats.CheckedChanged
		DisplaySICStatsForSelectedParentIon()
	End Sub

	Private Sub chkSortDescending_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSortDescending.CheckedChanged
		SortData()
	End Sub

	Private Sub chkShowSmoothedData_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
		UpdateStatsAndPlot()
	End Sub

	Private Sub chkUsePeakFinder_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkUsePeakFinder.CheckedChanged
		UpdateStatsAndPlot()
	End Sub
#End Region

#Region "Command Buttons"
	Private Sub cmdAutoStep_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAutoStep.Click
		ToggleAutoStep()
	End Sub
...
#Region "ListBoxes and Comboboxes"
	Private Sub lstParentIonData_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lstParentIonData.SelectedIndexChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub cboSICsTypeFilter_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboSICsTypeFilter.SelectedIndexChanged
		SortData()
	End Sub

	Private Sub cboSortOrder_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboSortOrder.SelectedIndexChanged
...
#Region "Option buttons"
	Private Sub optDoNotResmooth_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles optDoNotResmooth.CheckedChanged
...
#Region "Textboxes"
	...
	Private Sub txtAutoStep_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtAutoStep.TextChanged
	...
	Private Sub txtAutoStep_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtAutoStep.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtAutoStep, 10, 9999, 150)
	End Sub

	Private Sub txtButterworthSamplingFrequency_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtButterworthSamplingFrequency.TextChanged
		UpdateStatsAndPlot()
	End Sub
	...
	Private Sub txtFilterByMZ_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtFilterByMZ.Leave
		If chkFilterByMZ.Checked Then SortData()
	End Sub

	Private Sub txtFilterByMZ_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtFilterByMZ.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtFilterByMZ, 0, 100000, 540)
	End Sub

	Private Sub txtFilterByMZTol_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtFilterByMZTol.Leave
		If chkFilterByMZ.Checked Then SortData()
	End Sub

	Private Sub txtFilterByMZTol_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtFilterByMZTol.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxSng(txtFilterByMZTol, 0, 100000, 0.2)
	End Sub

	Private Sub txtFixXRange_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtFixXRange.TextChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub txtFixXRange_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtFixXRange.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtFixXRange, 3, 500000, 100)
	End Sub

	Private Sub txtFixYRange_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtFixYRange.TextChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub txtFixYRange_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtFixYRange.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtFixYRange, 10, Integer.MaxValue, 5000000)
	End Sub

	Private Sub txtMinimumIntensity_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMinimumIntensity.Leave
		If chkFilterByIntensity.Checked Then SortData()
	End Sub

	Private Sub txtMinimumIntensity_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtMinimumIntensity.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtMinimumIntensity, 0, 1000000000, 1000000)
	End Sub

	Private Sub txtMinimumSignalToNoise_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMinimumSignalToNoise.Leave
		If chkFilterBySignalToNoise.Checked Then SortData()
	End Sub

	Private Sub txtMinimumSignalToNoise_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtMinimumSignalToNoise.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtMinimumSignalToNoise, 0, 10000, 2)
	End Sub

	Private Sub txtPeakWidthPointsMinimum_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtPeakWidthPointsMinimum.TextChanged
		UpdateStatsAndPlot()
	End Sub

	Private Sub txtPeakWidthPointsMinimum_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtPeakWidthPointsMinimum.Validating
		SharedVBNetRoutines.VBNetRoutines.ValidateTextboxInt(txtPeakWidthPointsMinimum, 2, 100000, 6)
	End Sub

	Private Sub txtSavitzkyGolayFilterOrder_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtSavitzkyGolayFilterOrder.TextChanged
		UpdateStatsAndPlot()
	End Sub
	...
	Private Sub txtStats1_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtStats1.KeyPress
	...
	Private Sub txtStats2_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtStats2.KeyPress
		If Char.IsControl(e.KeyChar) Then
			Select Case Asc(e.KeyChar)
				Case 3
					' Ctrl+C; allow copy
	...
	Private Sub txStats3_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtStats3.KeyPress
		If Char.IsControl(e.KeyChar) Then
			Select Case Asc(e.KeyChar)
				Case 3
					' Ctrl+C; allow copy
...
#Region "Menubar"
	...
	Private Sub mnuEditShowOptimalPeakApexCursor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEditShowOptimalPeakApexCursor.Click
		mnuEditShowOptimalPeakApexCursor.Checked = Not mnuEditShowOptimalPeakApexCursor.Checked
	End Sub

	Private Sub mnuFileSelectMASICInputFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFileSelectMASICInputFile.Click
		SelectMASICInputFile()
	End Sub

	Private Sub mnuFileSelectMSMSSearchResultsFile_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFileSelectMSMSSearchResultsFile.Click
...
#End Region
	...
	Private Sub frmBrowser_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
		RegSaveSettings()
	End Sub
	...
	Private Sub tmrAutoStep_Elapsed(ByVal sender As Object, ByVal e As System.Timers.ElapsedEventArgs) Handles tmrAutoStep.Elapsed
		If mAutoStepEnabled Then CheckAutoStep()
	End Sub

	Private Sub frmBrowser_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Resize
		PositionControls()
	End Sub
