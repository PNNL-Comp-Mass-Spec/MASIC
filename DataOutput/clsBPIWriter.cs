...
namespace MASIC.DataOutput
{
    public class clsBPIWriter : clsMasicEventNotifier
    {
        ...
        private void SaveDecon2LSChromatogram(IEnumerable<clsScanInfo> scanList, clsSpectraCache spectraCache, string outputFilePath)
        {
            int scansWritten = 0;
        ...
        private void SaveICRToolsChromatogramByScan(clsMASICOptions masicOptions, IList<clsScanInfo> scanList, int scanCount, string outputFilePath, bool saveElutionTimeInsteadOfScan, bool saveTICInsteadOfBPI, string inputFilePathFull)
        {
            using (var writer = new StreamWriter(outputFilePath))
            {
        ...
        public void WriteDecon2LSIsosFileHeaders(StreamWriter writer)
        {
            ...
            var headerNames = new List<string>() { "scan_num", "charge", "abundance", "mz", "fit", "average_mw", "monoisotopic_mw", "mostabundant_mw", "fwhm", "signal_noise", "mono_abundance", "mono_plus2_abundance" };
            writer.WriteLine(string.Join(", ", headerNames));
        }

        public void WriteDecon2LSIsosFileEntry(StreamWriter writer, int scanNumber, int charge, double intensity, double ionMZ, float isoFit, double averageMass, double monoisotopicMass, double mostAbundantMass, float peakFWHM, double signalToNoise, float monoisotopicAbu, float monoPlus2Abu)
        {
            var dataValues = new List<string>() { scanNumber.ToString(), charge.ToString(), intensity.ToString("0.000"), ionMZ.ToString("0.00000"), isoFit.ToString(), averageMass.ToString("0.00000"), monoisotopicMass.ToString("0.00000"), mostAbundantMass.ToString("0.00000"), peakFWHM.ToString(), signalToNoise.ToString("0.000"), monoisotopicAbu.ToString(), monoPlus2Abu.ToString() };
        ...
        public void WriteDecon2LSScanFileHeaders(StreamWriter writer)
        {
            ...
            var headerNames = new List<string>() { "scan_num", "scan_time", "type", "bpi", "bpi_mz", "tic", "num_peaks", "num_deisotoped" };
            writer.WriteLine(string.Join(", ", headerNames));
        }
        ...
        public void WriteDecon2LSScanFileEntry(StreamWriter writer, clsScanInfo currentScan, int scanNumber, int msLevel, int numPeaks, int numIsotopicSignatures)
        {
            var dataLine = new List<string>() { scanNumber.ToString(), currentScan.ScanTime.ToString("0.0000"), msLevel.ToString(), DynamicValueToString(currentScan.BasePeakIonIntensity), currentScan.BasePeakIonMZ.ToString("0.00000"), DynamicValueToString(currentScan.TotalIonIntensity), numPeaks.ToString(), numIsotopicSignatures.ToString() };
